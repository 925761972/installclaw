...
// requireHelperLicense 校验 SaaS 会员权限。返回 true 表示已放行（或未启用），false 表示已拦截并已写错误响应。
// videoCount 为本次下载消耗的视频条数。
func (h *UploadHandler) requireHelperLicense(Conn *SunnyNet.HttpConn, videoCount int) bool {
	cfg := h.getConfig()
	if cfg == nil || !cfg.HelperEnabled {
		return true
	}
	client := cloud.NewLicenseClient(cfg)
	if !client.IsLoggedIn() {
		h.sendErrorResponse(Conn, fmt.Errorf("未登录会员账号，请到 Web 控制台登录后再下载"))
		return false
	}
	if _, err := client.Consume(videoCount); err != nil {
...
func (h *UploadHandler) HandleCompleteUpload(Conn *SunnyNet.HttpConn) bool {
	...
	// SaaS 会员校验（分片合并入库也需消费配额）
	if !h.requireHelperLicense(Conn, 1) {
		return true
	}
