...
func (c *Connector) processMessage(msg CloudMessage) {
	...
	// 云端转发的下载指令也需校验 SaaS 会员权限，避免绕过本地鉴权
	if cmd.Action == "download_video" && c.cfg != nil && c.cfg.HelperEnabled {
		client := NewLicenseClient(c.cfg)
		if !client.IsLoggedIn() {
			c.sendError(msg.ID, "未登录会员账号，无法执行云端下载指令")
			return
		}
		if _, err := client.Consume(1); err != nil {
			c.sendError(msg.ID, fmt.Sprintf("会员校验失败：%v", err))
			return
		}
	}
