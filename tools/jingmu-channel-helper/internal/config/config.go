...
type Config struct {
	...
	// SaaS 会员授权联动配置
	HelperEnabled  bool   `mapstructure:"helper_enabled"`   // 是否启用 SaaS 会员校验（严格模式：true 时未登录或配额不足禁止下载）
	HelperAPIBase  string `mapstructure:"helper_api_base"`  // SaaS 接口基址，例如 https://www.gname.vip
	HelperToken    string `mapstructure:"helper_token"`     // 持久化的 Bearer Token（登录后由 SaaS 颁发，存于本地）
	HelperUserID   string `mapstructure:"helper_user_id"`   // 已登录的 SaaS 用户 ID（用于本地展示）
	HelperEmail    string `mapstructure:"helper_email"`     // 已登录的 SaaS 邮箱（用于本地展示）
	HelperDeviceID string `mapstructure:"helper_device_id"` // 上报给 SaaS 的 deviceId，留空则用 MachineID
...
func setDefaults() {
	...
	viper.SetDefault("helper_device_id", "")
