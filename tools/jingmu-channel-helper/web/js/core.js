...
const ApiClient = {
    ...
    async startBatchDownload(videos, forceRedownload = false) {
        if (typeof ensureHelperDownloadAllowed === 'function') {
            const allowed = await ensureHelperDownloadAllowed(Array.isArray(videos) ? videos.length : 1);
            if (!allowed) {
                return { success: false, error: '请登录或开通视频号助手会员后继续下载' };
            }
        }
        const serviceUrl = ConnectionManager.getServiceUrl();
