...
async function loadDownloadQueue() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Completed queue items can use the local file as a durable preview.
            // Finder cover URLs expire, so enrich items from download records first.
            await Promise.all(queueState.items.map(async (item) => {
                if (item.status !== 'completed' || item.filePath) return;
                try {
                    const downloadRecord = await ApiClient.getDownloadRecord(item.videoId || item.id);
                    if (downloadRecord?.success && downloadRecord.data) {
                        item.filePath = downloadRecord.data.filePath || '';
                        item.coverUrl = downloadRecord.data.coverUrl || item.coverUrl || '';
                        item.duration = downloadRecord.data.duration || item.duration || 0;
                    }
                } catch (e) {
                    console.warn('Unable to enrich queue preview:', e);
                }
            }));
...
function renderQueueItem(item, index) {
    ...
    const preview = renderQueuePreview(item);
    ...
    return `
        <div class="queue-item" 
            ...
            <!-- Speed and ETA / completed file location -->
            ${item.status === 'completed' ? `
            <button type="button" class="queue-open-folder-btn" onclick="openQueueDownloadFolder('${escapeHtml(item.id)}')" title="在文件夹中定位视频">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 7h6l2 2h10v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7z"/><path d="M3 7V5a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v2"/></svg>
                <span>打开文件夹</span>
            </button>` : `
...
function renderQueuePreview(item) {
    const title = escapeHtml(item.title || '视频预览');
    const cover = escapeHtml(item.coverUrl || '');
    const duration = item.duration ? `<span class="queue-preview-duration">${formatDuration(item.duration)}</span>` : '';
    if (item.filePath) {
        return `<button type="button" class="queue-video-preview" onclick="previewQueueItem('${escapeHtml(item.id)}')" title="点击预览视频">
            ${cover ? `<img src="${cover}" alt="${title}" onerror="this.style.display='none';this.parentElement.classList.add('is-placeholder')">` : ''}
            <span class="queue-preview-placeholder">点击预览</span>
            <span class="queue-preview-play"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></span>${duration}
        </button>`;
    }
    if (item.coverUrl) {
        return `<button type="button" class="queue-video-preview" onclick="previewQueueItem('${escapeHtml(item.id)}')" title="点击预览视频">
            <img src="${cover}" alt="${title}" onerror="this.style.display='none';this.parentElement.classList.add('is-placeholder')">
            <span class="queue-preview-placeholder">暂无封面</span><span class="queue-preview-play"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></span>${duration}
        </button>`;
    }
    return `<div class="queue-video-preview is-placeholder"><span class="queue-preview-placeholder">暂无封面</span></div>`;
}

async function previewQueueItem(itemId) {
    const item = queueState.items.find(entry => entry.id === itemId);
    if (!item) return;
    if (item.filePath) {
        openVideoPlayer(item.filePath, item.title || '视频预览', true);
        return;
    }
    if (item.status === 'completed') {
        try {
            const record = await ApiClient.getDownloadRecord(item.videoId || item.id);
            if (record?.success && record.data?.filePath) {
                item.filePath = record.data.filePath;
                openVideoPlayer(item.filePath, item.title || record.data.title || '视频预览', true);
                return;
            }
        } catch (e) {
            console.warn('Unable to open local queue video:', e);
        }
    }
    if (item.coverUrl) {
        openLightbox(item.coverUrl, item.title || '视频封面', item.duration ? formatDuration(item.duration) : '');
    } else {
        showMessage('该任务暂无可用预览', 'error');
    }
}

async function openQueueDownloadFolder(itemId) {
    const item = queueState.items.find(entry => entry.id === itemId);
    if (!item) return;
    try {
        ...
        if (!filePath) {
            showMessage('未找到已下载的视频文件', 'error');
            return;
        }
        const result = await ApiClient.openFolder(filePath);
        if (result?.success === false) throw new Error(result.error || '打开失败');
        showMessage('已在文件夹中定位视频', 'success');
    } catch (e) {
