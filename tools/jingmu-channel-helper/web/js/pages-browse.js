...
let browseAutoRefreshTimer = null;
let browseAutoRefreshRunning = false;
let browseLastNotifiedId = '';
...
// Automatically detect newly captured videos while the browse page is visible.
// The probe only requests one row; the full table reloads only when data changed.
async function checkForNewBrowseRecords() {
    if (browseAutoRefreshRunning || document.visibilityState !== 'visible') return;
    if (typeof currentPage !== 'undefined' && currentPage !== 'browse') return;
    if (ConnectionManager.getStatus() !== 'connected') return;

    browseAutoRefreshRunning = true;
    try {
        const result = await ApiClient.getBrowseHistory({ page: 1, pageSize: 1 });
        const data = result?.data || {};
        const newest = data.items?.[0];
        const currentNewestId = browseState.records?.[0]?.id || '';
        const changed = (newest?.id && newest.id !== currentNewestId) || Number(data.total || 0) !== Number(browseState.totalCount || 0);
        if (!changed) return;

        const canRefreshInPlace = browseState.currentPage === 1 && !browseState.searchQuery && browseState.selectedIds.size === 0 && !browseState.currentDetailId;
        if (canRefreshInPlace) {
            await loadBrowseHistory();
            showBrowseLiveIndicator('已自动更新');
        } else if (newest?.id && newest.id !== browseLastNotifiedId) {
            browseLastNotifiedId = newest.id;
            showMessage('已抓取到新视频，清除筛选或返回第一页后自动显示', 'success');
        }
    } catch (e) {
        console.debug('Browse auto refresh probe failed:', e);
    } finally {
        browseAutoRefreshRunning = false;
    }
}

function showBrowseLiveIndicator(text = '实时更新中') {
    const page = document.getElementById('page-browse');
    if (!page) return;
    let indicator = page.querySelector('.browse-live-indicator');
    if (!indicator) {
        indicator = document.createElement('div');
        indicator.className = 'browse-live-indicator';
        indicator.innerHTML = '<span></span><b>实时更新中</b>';
        const toolbar = page.querySelector('.filter-bar, .browse-toolbar, .card-header');
        if (toolbar) toolbar.appendChild(indicator);
        else page.prepend(indicator);
    }
    const label = indicator.querySelector('b');
    if (label) label.textContent = text;
    indicator.classList.toggle('updated', text === '已自动更新');
    if (text === '已自动更新') {
        setTimeout(() => {
            if (label) label.textContent = '实时更新中';
            indicator.classList.remove('updated');
        }, 1800);
    }
}

function startBrowseAutoRefresh() {
    if (browseAutoRefreshTimer) return;
    browseAutoRefreshTimer = setInterval(checkForNewBrowseRecords, 2000);
    showBrowseLiveIndicator();
}

document.addEventListener('DOMContentLoaded', startBrowseAutoRefresh);
