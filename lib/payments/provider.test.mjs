...
function sortAndJoin(params, excludeSignType = true) {
...
test("alipay provider createPayment 返回电脑网站支付表单", async () => {
  ...
  try {
    ...
    assert.match(result.html, /\?app_id=app-123&amp;method=alipay\.trade\.page\.pay&amp;charset=utf-8/);
    assert.match(result.html, /accept-charset="UTF-8"/);
    assert.doesNotMatch(result.html, /name="charset"/);
    assert.match(result.html, /alipay\.trade\.page\.pay/);
    assert.match(result.html, /order-2/);
    assert.match(result.html, /19\.00/);

    const action = result.html.match(/action="([^"]+)"/)?.[1].replaceAll("&amp;", "&");
    assert.ok(action);
    const actionUrl = new URL(action);
    const requestParams = Object.fromEntries(actionUrl.searchParams.entries());
    const signature = requestParams.sign;
    assert.ok(signature);
    const bizContent = result.html.match(/name="biz_content" value="([^"]+)"/)?.[1];
    ...
    verifier.update(sortAndJoin(requestParams, false), "utf8");
    verifier.end();
    assert.equal(verifier.verify(keys.publicKey, signature, "base64"), true);
  } finally {
    restoreEnv();
  }
});
